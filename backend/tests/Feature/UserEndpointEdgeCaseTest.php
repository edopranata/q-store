...
class UserEndpointEdgeCaseTest extends TestCase
{
    ...
    {
        $response = $this->getJson('/api/v1/users?role=');
    ...
    public function test_handles_invalid_per_page_values()
    {
        ...
        $response = $this->getJson('/api/v1/users?per_page=1000');
        $response->assertStatus(422);
    }
    ...
    public function test_handles_invalid_date_formats()
    {
        ...
        $response = $this->getJson('/api/v1/users?created_from=invalid-date');
        $response->assertStatus(422);
        
        // Test invalid date range (created_to before created_from)
        $response = $this->getJson('/api/v1/users?created_from=2024-01-15&created_to=2024-01-10');
        $response->assertStatus(422);
    }
    ...
    public function test_handles_sql_injection_attempts()
    {
        ...
        $response = $this->getJson('/api/v1/users?search=' . urlencode("'; DROP TABLE users; --"));
        $response->assertStatus(200); // Should not cause error, just return empty results
        $this->assertTrue($response->json('success'));
        
        // Test SQL injection in role parameter
        $response = $this->getJson('/api/v1/users?role=' . urlencode("admin'; DROP TABLE roles; --"));
    ...
    public function test_handles_very_long_search_strings()
    {
        ...
        $response = $this->getJson('/api/v1/users?search=' . $longString);
        $response->assertStatus(422);
    }
    ...
    public function test_handles_special_characters_in_search()
    {
        ...
        $response = $this->getJson('/api/v1/users?search=' . urlencode('test@#$%^&*()'));
        $response->assertStatus(200);
        $this->assertTrue($response->json('success'));
    }
    ...
    public function test_handles_unicode_characters_in_search()
    {
        ...
        $response = $this->getJson('/api/v1/users?search=' . urlencode('测试用户'));
        $response->assertStatus(200);
        $this->assertTrue($response->json('success'));
    }
    ...
    public function test_handles_multiple_filters_combination()
    {
        ...
        $response = $this->getJson('/api/v1/users?role=admin&status=true&search=Admin&sort_by=name&sort_order=asc&per_page=5');
        $response->assertStatus(200);
        $this->assertTrue($response->json('success'));
    }
    ...
    public function test_handles_case_sensitivity_in_filters()
    {
        ...
        $response = $this->getJson('/api/v1/users?sort_order=ASC');
    ...
    public function test_performance_with_large_dataset()
    {
        ...
        $response = $this->getJson('/api/v1/users?per_page=20');
